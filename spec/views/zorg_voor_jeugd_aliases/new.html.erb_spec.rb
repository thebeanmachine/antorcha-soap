...
describe "/zorg_voor_jeugd_aliases/new.html.erb" do
  include ZorgVoorJeugdAliasesHelper

  before(:each) do
    assigns[:zorg_voor_jeugd_alias] = stub_model(ZorgVoorJeugdAlias,
      :new_record? => true,
      :organization_id => 1,
  ...
  it "renders new zorg_voor_jeugd_alias form" do
    render

    response.should have_tag("form[action=?][method=post]", zorg_voor_jeugd_aliases_path) do
